...
后续人工验收重点转为新默认 `world_main` 与下一批扩展：NPC 分散行动是否自然、弱化路径线与底部 tick 状态提示是否可读、右上角 `WorldPulsePanel` 是否能让玩家读懂 NPC 日程、HUD 暂停/倍速、三场景横向拼图、`E` 键 talk、VN 弹层、表情差分和真实 LLM profile 切换。玩家移动手感已由主人在 2026-05-17 确认没有问题。
