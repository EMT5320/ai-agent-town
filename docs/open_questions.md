...
- 玩家角色默认形象：偏少女年轻农场主。
- NPC 命名体系：游戏内显示名改为二次元轻幻想风，内部 ID 保持 `mira`、`tomas`、`orren`、`lena`、`kai`、`bram`。
...
- 奥伦·星历（`orren`）：退休教师，小镇历史和健康风险入口。
- 莉娜·白桦（`lena`）：医生，公共健康与理性冲突入口。
- 凯伊·月弦（`kai`）：酒馆乐手，社交、浪漫和债务冲突入口。
- 布拉姆·麦垄（`bram`）：农夫，农场主题和欠账冲突入口。
...
3. 第一批风格锁定资产生成后确认二次元轻幻想轻异世界是否作为长期主风格。
4. 星灯祭事件跑通后确认事件系统是否足以扩展到后续节日、委托和危机事件。
5. 偏少女玩家主角确认后，生成正式玩家半身立绘、表情差分和地图小人。
