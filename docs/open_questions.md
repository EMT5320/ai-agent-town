...
- 2026-05-16 主人已完成真实 Godot 窗口人工验收，基础通信、地点切换、NPC 选择、对话和事件展示基本可用。
- 下一轮验证重点：可移动地图、靠近交互、行动反馈和日程可视化是否能把体验从静态展示推进到可玩的生活模拟。
...
- 当前 Godot 已有持续按键移动、点击落点和靠近高亮；需要主人复验手感，并继续验证服务端锚点契约、行动反馈和日程可视化的最小结构。
