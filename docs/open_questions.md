...
- 本轮代码已补 WASD 独立移动、地图层直接点击当前场景空地落点、落点标记、当前场景角色/事件过滤、更大舞台移动范围、UI 点击穿透、单个最近目标高亮、玩家 / NPC 分离站位槽和收紧交互半径；这些体验仍需主人窗口复验。
...
- `gossipEvidence` 目前进入对话上下文并提供传播草案、debug summary 和 validator，后续是否写入公共事件、NPC 记忆或关系网络扩散。
...
- 当前 Godot 已有 WASD 独立移动、地图层空地点击落点、落点标记、当前场景过滤、更大舞台移动范围、UI 点击穿透、单目标靠近高亮、玩家 / NPC 分离站位槽和收紧交互半径；需要主人复验手感，并继续验证服务端锚点契约、行动反馈和日程可视化的最小结构。
