...
- Godot 4.x P0 客户端骨架（代码已接入，真实窗口人工未验收）。
...
| Godot 客户端 | 部分完成 | 代码已接入：地点背景层、NPC 选择、底部 VN 对话层、聊天提交、资源注册；命令已检：`client:run:check` DryRun 通过 | 人工未验收：真实窗口体验；事件交互只是文本展示，缺少 `inspect` / `attend_event` UI；地图小人和正式地图层尚未完成 |
| 资产管线 | 部分完成 | manifest 当前有 21 条资产；3 张背景、1 张事件 CG、玩家 + 6 NPC neutral 立绘已登记；11 条已填写 Godot `res://` 引用 | `happy` / `troubled` 表情差分、地图小人、道具图标、拆分 UI 组件尚未进 manifest 或 Godot registry |
...
- 主场景代码已能读取世界状态、渲染背景、列出地点和 NPC、展示半身立绘、提交聊天动作；真实窗口体验仍需人工验收。
...
- 当前已进入 Godot registry 的资产是 3 张地点背景和玩家 + 6 个首发 NPC 的 `neutral` 半身立绘。

## 4. 当前主要缺口

1. **真实 Godot 窗口验收**：需要用 `npm.cmd run start` + `npm.cmd run client:run` 人工确认窗口中的背景、NPC 选择、聊天提交、异常提示和同步频率。
2. **事件 UI**：Godot 只展示最近事件文本，仍需可点击事件卡、`inspect`、`attend_event`、选择结果和 VN 结算展示。
...
1. 先处理 Godot 事件 UI：把 `activeEvents` / `recentEvents` 变成可查看、可选择、可展示结果的 VN 交互。
2. 再收紧 Event Skill 数据化：用 Skill 定义驱动选项、后果预览、asset hints 和部分结算模板。
3. 配置一次真实 LLM smoke，确认 dialogue / event_reaction / night_reflection 的真实输出、延迟和 fallback。
4. 补 batch 1b 表情差分，并把新增资产写入 manifest 与 Godot registry。
5. Web Debug 追加 Director / Skill / fallback 视图，保持旧观察台不阻塞 Godot 主体验。