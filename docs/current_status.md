...
- 缺少正式游戏资产目录、Godot 导入清单、地图小人和角色立绘接入流程。
- 缺少二次元地图小人、半身立绘、表情差分、场景背景、事件 CG 和 UI 组件的实际资产。
...
- 角色卡、幻想显示名、视觉原型、说话风格、日程、喜好、冲突关系、恋爱铺垫标签和资产引用需要结构化。
...
2. 裁剪首版 6 NPC 和 3 地点数据，并补齐幻想显示名、视觉资产引用和恋爱铺垫字段。
3. 接入星灯祭供应短缺事件的 `attend_event` 链路。
