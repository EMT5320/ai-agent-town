...
| Godot 客户端 | 部分完成 | 代码已接入：地点背景层、NPC 选择、底部 VN 对话层、聊天提交、进行中事件区、`inspect`、choices、`attend_event`、VN 结果展示；地图角色层已渲染玩家与 6 个 NPC 的 `map_idle` 小人和 marker；新增方向键微移、点击地图落点、靠近高亮和 MapMoveHint；命令已检：`client:env` 与 `client:run:check` 通过；主人已完成真实窗口人工验收，基本无阻断问题 | 本地移动只做表现层；服务端锚点/交互半径契约、行动反馈、日程可视化和更自然的内容节奏仍待推进 |
...
- 主场景已新增本地地图移动与靠近反馈：方向键微移、点击地图设置落点、玩家小人平滑移动、靠近 NPC / 事件后高亮交互；该坐标不写回后端。
...
1. **玩法深度主线**：真实窗口已基本通过，本地移动与靠近反馈已落地；仍需要服务端锚点契约、行动反馈、生活行动循环和日程可视化。
...
2. Godot 玩法线继续推进：服务端锚点契约、行动反馈、生活行动按钮和日程可视化。
